...
fn main() {
    ...
    let preseeds = board[0].split(' ').map(|s| s.to_string().parse::<i64>().unwrap_or(-1)).filter(|&p| p != -1).collect::<Vec<_>>();
    let _ = board.remove(0);


    let fi = preseeds[0];    
    let la = fi + preseeds[1];
    ...
    for _ in  0..7
    {
        ...
        .map(|s| *s + array.iter()
                                .find_map(|(a,b,c)| if b <= &s && &(b + c) >= s {Some(a-b)} else {None} )
