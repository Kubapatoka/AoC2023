...
fn get_type(s: String) -> i32
{
    ...
    if tmp[tmp.len()-1] + jokers >= 5
    {
       return  6;
    }

    if tmp[tmp.len()-1] + jokers == 4
    {
        return 5;
    }

    if (tmp[tmp.len()-1] == 3 && tmp[tmp.len()-2] == 2) 
        || (tmp[tmp.len()-1] == 2 && tmp[tmp.len()-2] == 2 && jokers == 1)
    {
        return 4;
    }

    if tmp[tmp.len()-1] + jokers == 3
    {
        return 3;
    }
